...
int InitUninstControls() {
    ...
    if (!progressbar || !Nxt || !Bck || !useless || !waitAnimation || !restartWaitAnimation || !progressmeter || !Countdown) {
        MainLogger.WriteLine(L"One or more required uninstaller controls are missing.", -35);
        return -35;
    }
    ...
    for (int i = 0; i < 5; i++) {
        const std::wstring ws = L"chk1" + std::to_wstring(i + 1);
        auto* tch = (TouchCheckBox*)pMain->FindDescendent(StrToID((UCString)ws.c_str()));
        if (!tch) {
            MainLogger.WriteLine(L"Required uninstaller checkbox " + ws + L" was not found.", -36);
            return -36;
        }
...
int InitUninstaller() {
    ...
    err2 = InitUninstControls();
    if (FAILED(err2)) {
        MainLogger.WriteLine(L"One or more uninstaller controls failed to initialize correctly.\n", err2);
        return err2;
    }
